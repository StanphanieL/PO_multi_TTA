...
def train_epoch(train_loader, model, model_fn, optimizer, epoch, max_batch_iter):
    ...
    for k in am_dict.keys():
        if 'loss' == k:
            writer.add_scalar('train/loss', am_dict[k].avg, epoch)
    ...
    for i, batch in enumerate(train_loader):
        ...
        with torch.no_grad():
            if proto_module is not None:
                ...
                intra = (1 - (z_mean * own_proto).sum(dim=1)).mean().item()
        ...
        # collect for visualization (cap to cfg.emb_save_samples)
        try:
...
def SingleCard_training(cfgs):
    ...
    if cfg.contrastive or cfg.task == 'contrastive':
        ...
        # restore
        cfg.pretrain = ''
        start_epoch, pretrain_file = restore_contrastive_checkpoint(model, None, proto_module, cfg.logpath, pretrain_file=cfg.pretrain)
        ...
        for epoch in range(start_epoch, cfg.epochs):
            ...
            if improved:
                ...
                torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'prototypes': proto_module.state_dict()}, best_file)
    ...
    # optionally load contrastive backbone weights
